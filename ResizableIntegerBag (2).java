...
public class ResizableIntegerBag {
    ...
    public int add(int number) {
        if (numElements == 0) {
            bag = new Integer[DEFAULT_CAPACITY];
            bag[0] = number;
            numElements += 1;
            return 0;
        }
        // if numElements is equal to bag.length
        if (numElements == bag.length) {
            Integer[] tempArray = new Integer[bag.length * 2];
            for (int i = 0; i < bag.length; i++) {
                tempArray[i] = bag[i];
            }
            tempArray[bag.length] = number;
            bag = tempArray;
        }
        bag[numElements] = number;
        numElements += 1;
        return 0;
    }
    ...
    public void add(int index, int number) {
        ...
        // check index is last index of bag array if yes double the size

        if (index == (bag.length - 1)) {
            Integer[] copyBag = new Integer[bag.length * 2];
            bag = copyBag;
            if (copyBag[index] > number) {
                number = copyBag[index];
            }
            bag[index] = number;

        }
        int rPoint = index;
        int lPoint = bag.length - 1;
        int moveOne = 1 % bag.length;
        while (rPoint < lPoint) {
            int temp = bag[rPoint];
            bag[rPoint] = bag[lPoint];
            bag[lPoint] = temp;
            rPoint++;
            lPoint--;
        }
     ...
     */
    public int set(int index, int number) throws IndexOutOfBoundsException { //TODO sss
        if (index < 0 || index >= numElements) {
            throw new IndexOutOfBoundsException("Invalid index");
        }
        int prev = bag[index];
     ...
     * @throws IndexOutOfBoundsException if index < 0 or index >= size()
     */
    public int remove(int index) throws IndexOutOfBoundsException { //TODO
        if (index < 0 || index >= numElements) {
            throw new IndexOutOfBoundsException("Invalid index");
        }
        Integer[] temp = new Integer[bag.length-1];
